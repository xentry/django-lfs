...
def set_items_per_page(request):
    """Saves the given per page limit (by request body) to session.
    """
    limit = request.POST.get("items_per_page", "")
    if limit == "" and "items_per_page" in request.session:
        del request.session["items_per_page"]
    else:
        request.session["items_per_page"] = limit
